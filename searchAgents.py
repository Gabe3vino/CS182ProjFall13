...
class CornersProblem(search.SearchProblem):
  ...
  def isGoalState(self, state):
    ...
    c = self.corners
    a = state[1]
    return c[0] in a and c[1] in a and c[2] in a and c[3] in a 
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        pos = (nextx, nexty)
        if pos in self.corners and pos not in visited:
          successors.append(((pos, visited + [pos]), action, 1))
        else:
          successors.append(((pos, visited), action, 1))
...
def cornersHeuristic(state, problem):
  ...
  for i in range(len(corners)):
    if not corners[i] in visitedCorners:
...
def foodHeuristic(state, problem):
  ...
  min = 999999
  for order in itertools.permutations(foodGrid):
    dist = 0
    xy1 = pos
    for newPos in order:
      xy2 = newPos
